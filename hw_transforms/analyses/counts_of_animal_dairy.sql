with
    ...
    farm_counts as (
        select db_name, farm_id, count(farm_id) over farm_window as total_dairy_farms
    ...
    farm_bands as (
        select
            ...
            case
                when ac.total_animals_in_herd <= 100
                then '100 or less'
                when ac.total_animals_in_herd between 101 and 150
                then '100-150'
                when ac.total_animals_in_herd between 151 and 200
                then '151-200'
                when ac.total_animals_in_herd between 201 and 250
                then '201 and 250'
                when ac.total_animals_in_herd between 251 and 300
                then '251-300'
                when ac.total_animals_in_herd between 301 and 350
                then '301-350'
                when ac.total_animals_in_herd between 351 and 400
                then '351-400'
                when ac.total_animals_in_herd > 400
                then '400+'
            end as total_animals_bucket
        from animal_counts as ac
        inner join
            farm_counts as fc on ac.db_name = fc.db_name and ac.farm_id = fc.farm_id
    )
