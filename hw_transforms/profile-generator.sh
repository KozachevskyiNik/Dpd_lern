...
RUN_OPERATION_OUTPUT=$(dbt run-operation print_profile_docs --args '{"schema": "'$schema'", "exclude_measures":['min','max','avg','median','std_dev_population','std_dev_sample','distinct_proportion'], "relation_name": "'$relation_name'", "docs_name": "dbt_profiler_results__'$schema'_'$relation_name'"}')
